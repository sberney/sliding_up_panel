...
class _SlidingUpPanelState extends State<SlidingUpPanel>
  ...
  void _onGestureEnd(Velocity v) {
    ...
    // get the min distances AGAIN to ACTUALLY figure out where it is :)
    double closest;
    if (minDistance == d2Open)
      closest = 1.0;
    else if (minDistance == d2Snap)
      closest = widget.snapPoint;
    else
      closest = 0.0;

    print('y-position is: ${_ac.value}');
    print('y-velocity is: ${v.pixelsPerSecond.dy}');

    //if (v.pixelsPerSecond.dy <= 0.0001) {
    if (v.pixelsPerSecond.dy == 0) {
      print('velocity is zero, so snapping to closest point by position');
      _flingPanelToPosition(closest, visualVelocity);
    } else {
      print('velocity is non-zero, so snapping to a logical position');
      // down (to close) is a positive velocity for some infernal reason.   -- perhaps because on iOS positive velocity means "towards spring attachment point"
      // down (to close) is a negative positional displacement, as expected, however.

      bool superFling = v.pixelsPerSecond.dy.abs() >= kSnap * minFlingVelocity;
      ...
      if (!superFling) {
        ...
        if (yVel > 0) {
          // going down
          print('going down');
          if (yPos > widget.snapPoint)
            target = widget.snapPoint;
          else
            target = 0;
        } else {
          // going up
        ...
        // super fling -- skip any snap points
        print(
